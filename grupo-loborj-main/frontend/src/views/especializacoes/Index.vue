...
import { useDeleteEspecializacaoMutation, useEspecializacoesQuery, type Especializacao } from '@/composables/useEspecializacoes';

const { setHeader } = usePage();
setHeader('Especializações e Requisitos', 'Configure as opções de especialização (para técnicos) e requisitos (para serviços e contratos).');
...
const { data: especializacoesResponse, isLoading, isFetching } = useEspecializacoesQuery(queryParams);

// --- MUTAÇÕES (DELETAR / RESTAURAR) ---
const deleteMutation = useDeleteEspecializacaoMutation();
...
async function handleDelete(especializacao: Especializacao) {
  openConfirmModal({
    title: 'Excluir Especialização',
    message: `Tem certeza que deseja excluir a especialização/requisito ${especializacao.name}? Esta operação não pode ser desfeita.`,
    confirmText: 'Excluir',
    type: 'danger',
    onConfirm: async () => {
      modalState.isLoading = true;
      try {
        await deleteMutation.mutateAsync(especializacao.id);
        toastStore.send({ message: 'Especialização excluída com sucesso.', style: 'success' });
        modalState.isOpen = false;
      } catch (error) {
        const err = error as any;
        toastStore.send({ message: err?.errors ? Object.values(err.errors).flat().join(' ') : err?.message ?? 'Erro interno ao tentar excluir especialização.', style: 'error' });
      } finally {
...
<template>
  <div class="space-y-6 py-3">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <h2 class="text-xl font-bold text-slate-800">Lista de Especializações e Requisitos</h2>
      <button title="Criar novo tipo de especialização/requisito" v-can="'GERIR_CONFIGURACOES'"
        @click="router.push({ name: 'especializacoes.create' })" type="button"
      ...
      <div class="w-full sm:w-1/3">
        ...
        <div class="mt-1 relative rounded-md">
          ...
          <input v-model.lazy="queryParams.search" type="text" id="search"
            ...
            placeholder="Nome da especialização/requisito">
        </div>
      </div>
    ...
    <!-- Tabela de Dados -->
    <DataTable :columns="columns" :items="especializacoesResponse?.data || []" :loading="isLoading"
      :pagination="especializacoesResponse?.meta" :sort-by="queryParams.sort_by" :sort-dir="queryParams.sort_dir"
      empty-message="Nenhuma especialização foi encontrada" @sort="handleSort" @update:page="queryParams.page = $event"
      @update:per-page="queryParams.per_page = $event">
      <!-- Slot customizado: Nome -->
      <template #cell-name="{ item }">
        <div class="px-2">
          <p class="text-base font-medium text-primary">{{ item.name }}</p>
          <p class="text-slate-500 text-xs max-w-xs text-wrap">{{ item.users_count }} funcionário{{ item.users_count ==
            1 ? '' : 's' }} com esta
            especialização</p>
        </div>
      </template>
      ...
      <template #cell-actions="{ item }">
        <div class="flex items-center justify-end gap-3">
          ...
          <button v-can="'GERIR_CONFIGURACOES'"
            @click="router.push({ name: 'especializacoes.edit', params: { id: item.id } })" title="Editar"
          ...
          <button v-can="'GERIR_CONFIGURACOES'" @click="handleDelete(item as Especializacao)" title="Excluir"
