...
import { useDeleteRoleMutation, useRolesQuery, type Role } from '@/composables/useRoles';

const { setHeader } = usePage();
setHeader('Cargos', 'Gerencie os perfis de acesso e suas permissões na plataforma.');
...
const { data: rolesResponse, isLoading, isFetching } = useRolesQuery(queryParams);
...
async function handleDelete(role: Role) {
  openConfirmModal({
    title: 'Excluir Cargo',
    message: `Tem certeza que deseja excluir o cargo ${role.name}? Esta operação não pode ser desfeita.`,
    confirmText: 'Excluir',
    type: 'danger',
    onConfirm: async () => {
      ...
      try {
        ...
        toastStore.send({ message: 'Cargo excluído com sucesso.', style: 'success' });
        modalState.isOpen = false;
      } catch (error) {
        const err = error as any;
        toastStore.send({ message: err?.errors ? Object.values(err.errors).flat().join(' ') : err?.message ?? 'Erro interno ao tentar excluir cargo.', style: 'error' });
      } finally {
...
<template>
  <div class="space-y-6 py-3">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <h2 class="text-xl font-bold text-slate-800">Lista de Cargos</h2>
      <button v-can="'GERIR_CONFIGURACOES'" @click="router.push({ name: 'roles.create' })" type="button"
    ...
    <!-- Tabela de Dados -->
    <DataTable :columns="columns" :items="rolesResponse?.data || []" :loading="isLoading"
      :pagination="rolesResponse?.meta" :sort-by="queryParams.sort_by" :sort-dir="queryParams.sort_dir"
      empty-message="Nenhum cargo foi encontrado" @sort="handleSort" @update:page="queryParams.page = $event"
      ...
      <!-- Slot customizado: Permissões -->
      <template #cell-permissions="{ item }">
        <div class="flex flex-col gap-1">
          <p v-if="item.permissions.length === 0" class="text-slate-400 text-xs">Sem permissões</p>
          <ul v-else class="list-disc grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3">
            <li v-for="user in item.permissions" :key="user.email" class="text-sm font-medium text-slate-700">
              {{ user.name }}
            </li>
          </ul>
        </div>
      ...
      <template #cell-actions="{ item }">
        <div class="flex items-center justify-end gap-3">
          <!-- Botão Editar -->
          <button v-can="'GERIR_CONFIGURACOES'" @click="router.push({ name: 'roles.edit', params: { id: item.id } })"
            title="Editar"
          ...
          <button v-can="'GERIR_CONFIGURACOES'" @click="handleDelete(item as Role)" title="Excluir"
