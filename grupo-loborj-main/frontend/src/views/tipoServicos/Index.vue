...
import { useDeleteTipoServicoMutation, useTipoServicosQuery, type TipoServico } from '@/composables/useTipoServicos';

const { setHeader } = usePage();
setHeader('Tipos de Serviço', 'Configure as opções de tipos de serviço para os serviços e contratos.');
...
const { data: tipoServicosResponse, isLoading, isFetching } = useTipoServicosQuery(queryParams);

// --- MUTAÇÕES (DELETAR / RESTAURAR) ---
const deleteMutation = useDeleteTipoServicoMutation();

// --- CONFIGURAÇÃO DAS COLUNAS DA TABELA ---
const columns: Column[] = [
  { key: 'name', label: 'Tipo de Serviço', sortable: true },
...
async function handleDelete(tipoServico: TipoServico) {
  openConfirmModal({
    title: 'Excluir Tipo de Serviço',
    message: `Tem certeza que deseja excluir o tipo de serviço ${tipoServico.name}? Esta operação não pode ser desfeita.`,
    confirmText: 'Excluir',
    type: 'danger',
    onConfirm: async () => {
      modalState.isLoading = true;
      try {
        await deleteMutation.mutateAsync(tipoServico.id);
        toastStore.send({ message: 'Tipo de serviço excluído com sucesso.', style: 'success' });
        modalState.isOpen = false;
      } catch (error) {
        const err = error as any;
        toastStore.send({ message: err?.errors ? Object.values(err.errors).flat().join(' ') : err?.message ?? 'Erro interno ao tentar excluir tipo de serviço.', style: 'error' });
      } finally {
...
<template>
  <div class="space-y-6 py-3">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <h2 class="text-xl font-bold text-slate-800">Lista de Tipos de Serviço</h2>
      <button title="Criar novo tipo de serviço" v-can="'GERIR_CONFIGURACOES'"
        @click="router.push({ name: 'tipoServicos.create' })" type="button"
    ...
    <!-- Tabela de Dados -->
    <DataTable :columns="columns" :items="tipoServicosResponse?.data || []" :loading="isLoading"
      :pagination="tipoServicosResponse?.meta" :sort-by="queryParams.sort_by" :sort-dir="queryParams.sort_dir"
      empty-message="Nenhum tipo de serviço foi encontrado" @sort="handleSort" @update:page="queryParams.page = $event"
      ...
      <template #cell-actions="{ item }">
        <div class="flex items-center justify-end gap-3">
          ...
          <button v-can="'GERIR_CONFIGURACOES'"
            @click="router.push({ name: 'tipoServicos.edit', params: { id: item.id } })" title="Editar"
          ...
          <button v-can="'GERIR_CONFIGURACOES'" @click="handleDelete(item as TipoServico)" title="Excluir"
