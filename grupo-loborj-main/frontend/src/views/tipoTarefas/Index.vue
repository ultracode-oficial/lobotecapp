...
import { useDeleteTipoTarefaMutation, useTipoTarefasQuery, type TipoTarefa } from '@/composables/useTipoTarefas';

const { setHeader } = usePage();
setHeader('Tipos de Tarefa', 'Configure as opções de tipos de tarefas.');
...
const { data: tipoTarefasResponse, isLoading, isFetching } = useTipoTarefasQuery(queryParams);

// --- MUTAÇÕES (DELETAR / RESTAURAR) ---
const deleteMutation = useDeleteTipoTarefaMutation();

// --- CONFIGURAÇÃO DAS COLUNAS DA TABELA ---
const columns: Column[] = [
  { key: 'name', label: 'Tipo de Tarefa', sortable: true },
  { key: 'recurrence_days', label: 'Retorno Sugerido (em dias)', sortable: false },
...
async function handleDelete(tipoTarefa: TipoTarefa) {
  openConfirmModal({
    title: 'Excluir Tipo de Tarefa',
    message: `Tem certeza que deseja excluir o tipo de tarefa ${tipoTarefa.name}? Esta operação não pode ser desfeita.`,
    confirmText: 'Excluir',
    type: 'danger',
    onConfirm: async () => {
      modalState.isLoading = true;
      try {
        await deleteMutation.mutateAsync(tipoTarefa.id);
        toastStore.send({ message: 'Tipo de tarefa excluído com sucesso.', style: 'success' });
        modalState.isOpen = false;
      } catch (error) {
        toastStore.send({ message: 'Erro ao excluir tipo de tarefa.', style: 'error' });
      } finally {
...
<template>
  <div class="space-y-6 py-3">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
      <h2 class="text-xl font-bold text-slate-800">Lista de Tipos de Tarefa</h2>
      <button title="Criar novo tipo de tarefa" v-can="'GERIR_CONFIGURACOES'"
        @click="router.push({ name: 'tipoTarefas.create' })" type="button"
    ...
    <!-- Tabela de Dados -->
    <DataTable :columns="columns" :items="tipoTarefasResponse?.data || []" :loading="isLoading"
      :pagination="tipoTarefasResponse?.meta" :sort-by="queryParams.sort_by" :sort-dir="queryParams.sort_dir"
      empty-message="Nenhum tipo de tarefa foi encontrado" @sort="handleSort" @update:page="queryParams.page = $event"
      ...
      <template #cell-recurrence_days="{ item }">
        <div class="text-base max-w-xs text-wrap"
          :class="item.recurrence_days == null ? 'text-slate-400' : 'text-slate-600'">{{ item.recurrence_days ?
            `${item.recurrence_days} dias` : "N/A" }}</div>
      </template>
      ...
      <template #cell-actions="{ item }">
        <div class="flex items-center justify-end gap-3">
          ...
          <button v-can="'GERIR_CONFIGURACOES'"
            @click="router.push({ name: 'tipoTarefas.edit', params: { id: item.id } })" title="Editar"
          ...
          <button v-can="'GERIR_CONFIGURACOES'" @click="handleDelete(item as TipoTarefa)" title="Excluir"
