...
watch(() => props.initialData, (data) => {
  if (data) {
    form.name = data.name || '';
    form.permissions = data.permissions?.map((u: any) => u.id) || [];
  }
}, { immediate: true });
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-8">
    <div class="space-y-5">
      ...
      <div>
        <div>
          <h3 class="text-lg font-medium leading-6 text-gray-900">Dados do Cargo</h3>
          <p class="mb-5 text-sm text-gray-500">Defina o nome deste cargo.</p>
        </div>
        <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
          <div class="sm:col-span-4">
            <label class="block text-sm font-medium text-slate-700 mb-1">Nome do Cargo <span
                class="font-bold">*</span></label>
            <input v-model="form.name" type="text" required placeholder="Ex.: ADMINISTRADOR"
      ...
      <div class="pt-4">
        <div>
          ...
          <p class="mt-1 text-sm text-gray-500">Selecione as permissões que pertencem a este cargo.</p>
        </div>
        <div class="mt-6">
          <div v-if="permissions?.length === 0" class="text-sm text-gray-500 p-4 bg-gray-50 rounded-md">
            Nenhuma permissão foi encontrada no sistema.
          </div>
          <div v-else
            class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 border border-gray-200 rounded-md p-4 max-h-64 overflow-y-auto">
